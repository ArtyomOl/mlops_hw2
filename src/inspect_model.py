...
def device_allocated_bytes(device: torch.device) -> int:
    """Метрика памяти выбранного устройства в байтах."""
    if device.type == "mps":
...
class PeakMemory:
    """Пик за шаг; на MPS — максимум выборок с заданным интервалом."""
    ...
    def checkpoint(self, name: str) -> None:
        ...
        self.sample()
        self.stages[name] = round(device_allocated_bytes(self.device) / 1024 ** 2, 1)
...
def measure_mode(mode: str, params: dict) -> dict:
    ...
    gradient_bytes = optimizer_bytes = adapter_bytes = 0
    ...
    with PeakMemory(device) as peak:
        ...
        if mode == "inference":
            ...
            if mode == "lora":
                ...
                adapter_bytes = sum(p.numel() * p.element_size() for p in model.parameters()
                                    if p.requires_grad)
            model.train()
...
def memory_profile(params: dict) -> list[dict]:
    ...
    for mode in MODES:
        ...
        for _ in range(repeats):
            ...
            if proc.returncode:
                raise RuntimeError(f"Замер {mode} завершился с кодом {proc.returncode}:\n{proc.stderr}")
...
def main() -> None:
    ...
    training_before = [m.training for m in model.modules()]
    ...
    assert activations == repeated
    assert hooks_before == hooks_after
    assert training_before == [m.training for m in model.modules()]
    ...
    report = {
        ...
        "hook_check": {"before": hooks_before, "after": hooks_after,
                       "repeat_equal": activations == repeated, "training_restored": True},
    ...
    assert peaks["full_ft"] > peaks["lora"] > peaks["inference"]
    assert all(item["peak_mb"] >= item["weights_bytes"] / 1024 ** 2 for item in memory)
